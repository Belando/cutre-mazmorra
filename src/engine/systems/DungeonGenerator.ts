...
function getSpriteForEnemy(type: number | string): SpriteComponent | null {
...
export function generateDungeon(width: number, height: number, level: number, playerLevel = 1): DungeonResult {
  ...
  if (entitiesGrid[bossY + bossOffsetY]) {
    entitiesGrid[bossY + bossOffsetY][bossX + bossOffsetX] = bossType;
  }
