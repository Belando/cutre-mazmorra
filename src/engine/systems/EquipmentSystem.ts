...
export function canEquipItem(item: Item | null | undefined, playerClass: string): boolean {
    ...
    if (slot === 'ring' || slot === 'earring' || slot === 'necklace') {
        return true;
    }
...
export function calculateEquipmentStats(equipment: EquipmentState | undefined): Stats {
    ...
    Object.values(equipment || {}).forEach(item => {
        if (item?.stats) {
            if (item.stats.attack) stats.attack = (stats.attack || 0) + item.stats.attack;
            if (item.stats.defense) stats.defense = (stats.defense || 0) + item.stats.defense;
            if (item.stats.maxHp) stats.maxHp = (stats.maxHp || 0) + item.stats.maxHp;
            if (item.stats.evasion) stats.evasion = (stats.evasion || 0) + item.stats.evasion;
            if (item.stats.critChance) stats.critChance = (stats.critChance || 0) + item.stats.critChance;
            if (item.stats.magicPower) stats.magicPower = (stats.magicPower || 0) + item.stats.magicPower;
        }
    });
...
export function generateEquipment(type: string, slot: EquipmentSlot, rarity: Rarity, dungeonLevel: number): { stats: Stats; rarity: Rarity } {
    ...
    const baseStats: Record<string, Partial<Stats>> = {
...
export const ARMOR_TEMPLATES: Record<string, Record<string, string[]>> = {
    heavy: {
...
export const WEAPON_TEMPLATES: Record<string, Record<Rarity, string[]>> = {
    sword: {
