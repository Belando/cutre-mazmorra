...
export function getEuclideanDistance(a: Point, b: Point): number {
    return Math.sqrt(Math.pow(a.x - b.x, 2) + Math.pow(a.y - b.y, 2));
}

// Comprobar si el objetivo está dentro del rango
export function isInRange(attacker: Point, target: Point, range: number): boolean {
...
export interface ProjectileStep extends Point {
    distance: number;
}

export function getProjectilePath(x1: number, y1: number, x2: number, y2: number, map: number[][], maxRange: number = 8): ProjectileStep[] {
