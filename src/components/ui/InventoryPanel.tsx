...
const getClassRestrictionText = (item: Item) => {
    let classes: string[] = [];
    if (item.weaponType && (WEAPON_TYPES as any)[item.weaponType]) {
        classes = (WEAPON_TYPES as any)[item.weaponType].classes;
    } else if (item.armorType && (ARMOR_TYPES as any)[item.armorType]) {
        classes = (ARMOR_TYPES as any)[item.armorType].classes;
    }
...
const ItemSlot = ({ item, onClick, isSelected, isEmpty, index, onDragStart, onDrop, onDragOver }: ItemSlotProps) => {
    if (isEmpty || !item) {
        return (
            <div
...
const EquipSlot = ({ slotKey, equipment, onSelect, isSelected }: EquipSlotProps) => {
...
const ItemIconWrapper = ({ item }: { item: Item }) => {
    const Icon = getItemIcon(item);
    return <Icon />;
};
                    ...
                    {!selectedItem ? (
                        ...
                        <>
                            <div className="p-6 pb-4 border-b border-slate-800 bg-gradient-to-b from-slate-900 to-slate-950">
                                ...
                                <div className="text-center">
                                    ...
                                    <span className="block mt-1 text-xs tracking-widest uppercase text-slate-500">
                                        {selectedItem.rarity} • {selectedItem.category === 'weapon' ? ((WEAPON_TYPES as any)[selectedItem.weaponType]?.name || selectedItem.category) : selectedItem.category}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 p-6 overflow-y-auto">
                                ...
                                {selectedItem.slot && (EQUIPMENT_SLOTS as any)[selectedItem.slot] && (
                                    <div className="flex items-center gap-3 p-3 mb-4 border rounded-lg bg-slate-900/80 border-slate-700">
                                        <div className="flex items-center justify-center w-10 h-10 border rounded bg-slate-800 border-slate-600">
                                            {React.createElement((EQUIPMENT_SLOTS as any)[selectedItem.slot].icon, { className: "w-6 h-6 text-slate-400" })}
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-slate-500 uppercase font-bold">
                                                {selectedItem.isEquipped ? "Equipado en" : "Se equipa en"}
                                            </p>
                                            <p className="text-sm font-medium text-blue-300">{(EQUIPMENT_SLOTS as any)[selectedItem.slot].name}</p>
                                        </div>
