...
import { Item, Entity, GameState } from '@/types';

function getSellPrice(item: Item) {
    const rarityMultiplier: Record<string, number> = { common: 5, uncommon: 15, rare: 40, epic: 100, legendary: 250 };
    const base = rarityMultiplier[item.rarity as string] || 5;
    const statBonus = ((item.stats as any)?.attack || 0) * 3 + ((item.stats as any)?.defense || 0) * 3 + ((item.stats as any)?.maxHp || 0);
...
const IconWrapper = ({ item, className }: { item: Item, className?: string }) => {
...
interface NPCDialogProps {
    ...
    onAcceptQuest: (quest: any) => void;
    onCompleteQuest: (quest: any) => void;
    activeQuests?: string[];
...
export default function NPCDialog({
    ...
    const [activeMode, setActiveMode] = useState<'shop' | 'sell' | 'quests' | 'lore' | null>(null);
    ...
    const handleSell = (i: number, price: number) => {
        onSell(i, price);
                                ...
                                <div className="grid gap-2">
                                    {inventory.map((item, i) => {
                                        ...
                                        return (
                                            <div key={i} className="flex items-center justify-between p-2 transition-colors border rounded-lg bg-slate-800/40 border-slate-700/50 hover:bg-slate-800 hover:border-slate-600">
                                                <div className="flex items-center gap-3">
                                                    <div className={`p-2 rounded-md bg-slate-900 border border-slate-700 ${getRarityTextColor(item.rarity as string)}`}>
                                                        <IconWrapper item={item} className="w-5 h-5" />
                                                    </div>
                                                    <div>
                                                        <div className={`text-sm font-medium ${getRarityTextColor(item.rarity as string)}`}>{item.name} {(item.quantity || 1) > 1 && `x${item.quantity}`}</div>
