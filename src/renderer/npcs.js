  ...
  blacksmith: {
    draw: (ctx, x, y, size, frame) => {
      ...
      const c = {
        ...
        shirt: "#d4d4d8", 
        metalDark: "#334155",
        metalMid: "#475569", // Un poco más oscuro para facetas
        metalLight: "#94a3b8",
        metalHighlight: "#cbd5e1", // Nuevo brillo intenso
        stoneDark: "#292524",
        stoneMid: "#44403c",
        stoneLight: "#78716c",
      ...
      // ==========================================
      // -- Estructura Principal con Ladrillos --
      ctx.fillStyle = c.stoneDark; // Fondo oscuro (cemento)
      ctx.fillRect(forgeX, forgeY, s*1.1, s*0.95);
      ctx.fillStyle = c.stoneMid; // Ladrillos
      for(let i=0; i<4; i++) { // 4 filas
          let off = (i%2===0) ? 0 : s*0.1; // Desplazamiento alterno
          ctx.fillRect(forgeX + off + s*0.05, forgeY + i*s*0.2 + s*0.05, s*0.35, s*0.15);
          ctx.fillRect(forgeX + off + s*0.5, forgeY + i*s*0.2 + s*0.05, s*0.35, s*0.15);
      }

      // -- Boca y Fuego (Igual que antes) --
      ...
      // -- Chimenea Detallada y Humo --
      ctx.fillStyle = c.stoneDark;
      ctx.fillRect(forgeX + s*0.35, forgeY - s*0.3, s*0.4, s*0.4);
      ctx.fillStyle = c.stoneMid; // Ladrillos chimenea
      ctx.fillRect(forgeX + s*0.4, forgeY - s*0.25, s*0.3, s*0.1);
      ctx.fillRect(forgeX + s*0.35, forgeY - s*0.1, s*0.3, s*0.1);
      
      ctx.fillStyle = c.smoke; // Humo (Igual que antes)
      for(let i=0; i<4; i++) {
          const smokeOffset = (frame * 1.5 + i * 30) % 120; const t = smokeOffset / 120;
          const smokeY = (forgeY - s*0.3) - (smokeOffset * s * 0.015); const smokeX = (forgeX + s*0.55) + Math.sin(smokeOffset * 0.08) * s * 0.15 * t;
          ctx.globalAlpha = 1 - t; ctx.beginPath(); ctx.arc(smokeX, smokeY, s * (0.1 + t * 0.2), 0, Math.PI*2); ctx.fill(); ctx.globalAlpha = 1.0;
      }

      // ==========================================
      // 2. YUNQUE CLÁSICO DETALLADO (Medio)
      // ==========================================
      ...
      // --> YUNQUE DETALLADO (Volumen 3D) <--
      // Base y Cuerpo (Metal Oscuro - Sombra)
      ctx.fillStyle = c.metalDark;
      ctx.fillRect(anvilX - s*0.1, anvilY + s*0.2, s*0.6, s*0.05); // Base inferior
      ctx.beginPath(); // Cuerpo principal y sombra del cuerno
      ctx.moveTo(anvilX - s*0.25, anvilY + s*0.02); 
      ctx.lineTo(anvilX + s*0.5, anvilY + s*0.02);
      ctx.lineTo(anvilX + s*0.45, anvilY + s*0.2);
      ctx.lineTo(anvilX + s*0.05, anvilY + s*0.2);
      ctx.fill();

      // Cara lateral (Metal Medio)
      ctx.fillStyle = c.metalMid;
      ctx.fillRect(anvilX, anvilY + s*0.02, s*0.5, s*0.1); // Lateral recto
      // Lateral del cuerno (curvo)
      ctx.beginPath();
      ctx.moveTo(anvilX, anvilY + s*0.02);
      ctx.lineTo(anvilX, anvilY + s*0.12);
      ctx.quadraticCurveTo(anvilX - s*0.15, anvilY + s*0.08, anvilX - s*0.25, anvilY + s*0.02);
      ctx.fill();

      // Superficie Superior (Metal Claro - Luz)
      ctx.fillStyle = c.metalLight;
      ctx.beginPath();
      ctx.moveTo(anvilX - s*0.25, anvilY); // Punta cuerno
      ctx.lineTo(anvilX + s*0.5, anvilY); // Final derecho
      ctx.lineTo(anvilX + s*0.5, anvilY + s*0.05);
      ctx.lineTo(anvilX + s*0.05, anvilY + s*0.05); // Inicio talón
      ctx.quadraticCurveTo(anvilX, anvilY + s*0.08, anvilX - s*0.25, anvilY); // Curva superior cuerno
      ctx.fill();
      
      // Brillo intenso borde superior
      ctx.fillStyle = c.metalHighlight; ctx.fillRect(anvilX, anvilY, s*0.5, s*0.02);
      
      // Hardy Hole (Agujero cuadrado para herramientas)
      ctx.fillStyle = c.metalDark;
      ctx.fillRect(anvilX + s*0.35, anvilY + s*0.01, s*0.06, s*0.06);

      // ==========================================
      // 3. EL HERRERO - CAPA TRASERA (BRAZO DERECHO)
      // ==========================================
      ...
      // --- DIBUJO BRAZO DERECHO CON MARTILLO CORTO ---
      ctx.save();
      ...
      // Antebrazo
      ctx.translate(s*0.25, 0); ctx.rotate(forearmRot); ctx.fillRect(0, -s*0.05, s*0.22, s*0.12);
      
      // --> MARTILLO DE MANGO CORTO <--
      ctx.translate(s*0.18, 0); ctx.rotate(Math.PI/2);
      // Mango corto (Empieza más abajo, acaba antes)
      ctx.fillStyle = c.wood; ctx.fillRect(-s*0.05, -s*0.15, s*0.1, s*0.22);
      // Cabeza (Reajustada posición al nuevo mango)
      ctx.fillStyle = c.metalDark; ctx.fillRect(-s*0.12, -s*0.3, s*0.24, s*0.15);
      ctx.fillStyle = c.metalLight; ctx.fillRect(-s*0.12, -s*0.17, s*0.24, s*0.03);
      ctx.restore();

      // ==========================================
      // 4. EL HERRERO - CAPA DELANTERA (CUERPO)
      // ==========================================
      // (Todo esto es exactamente igual que antes)
      // -- Piernas --
      ...
      // -- Torso y Delantal --
      ctx.fillStyle = c.shirt; ctx.fillRect(smithX + s*0.05, smithY + s*0.15 + breath, s*0.35, s*0.5);
      ...
      ctx.fillRect(smithX + s*0.05, smithY - s*0.08 + breath, s*0.3, s*0.1); // Pelo
      ctx.fillStyle = "#000"; ctx.fillRect(smithX + s*0.3, smithY + s*0.05 + breath, s*0.04, s*0.03); // Ojo
      // -- Brazo Izquierdo (Tenazas - Primer plano) --
      ctx.save(); ctx.translate(smithX + s*0.25, smithY + s*0.2 + breath); ctx.rotate(0.3);
      ctx.fillStyle = c.skin; ctx.fillRect(0, 0, s*0.25, s*0.1);
      ctx.translate(s*0.25, 0); ctx.rotate(0.4); ctx.fillRect(0, 0, s*0.2, s*0.08);
      ctx.fillStyle = c.metalDark; ctx.fillRect(s*0.15, -s*0.05, s*0.3, s*0.05); ctx.fillRect(s*0.15, s*0.05, s*0.3, s*0.05); ctx.fillRect(s*0.45, -s*0.05, s*0.05, s*0.15);
      ctx.restore();
